...
import { db, auth, firebaseApp } from './components/firebaseConfig';
    ...
    useEffect(() => {
        if (!firebaseAppInstance) {
            try {
                ...
                setFirebaseAppInstance(firebaseApp);
                setDbInstance(db);
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
                        {userId ? (
                            <>
                                ...
                                <Link to="/admin" className="px-4 py-2 bg-blue-600 text-white rounded-xl text-sm hover:bg-blue-700 transition shadow-lg">Admin Page</Link>
                            </>
                        ) : (
...
function App() {
    return (
        <Router>
            <Routes>
                <Route path="/" element={<BookingAppContent />} />
...
export default BookingAppContent;
