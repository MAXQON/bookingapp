...
app.post('/api/confirm-booking', verifyFirebaseToken, async (req, res) => {
    ...
    try {
        ...
        if (calendar && googleCalendarId) {
            ...
            // Construct start and end dates using local time components
            // This prevents Date() from interpreting the string as UTC or local time zone implicitly
            // We explicitly set the timezone in the event object for Google Calendar
            const startDate = new Date(year, month - 1, day, hour, minute); // month-1 because months are 0-indexed
...
You're right to point out that the time display is still off, and now it's even jumping across days, which is definitely worse! The issue stems from an incorrect handling of timezones when creating `Date` objects in your backend.

The previous attempt to include `+08:00` in the string passed to `new Date()` can be problematic because `new Date()` without specific parsing often defaults to parsing in local time or UTC, and then applies the offset *again*, leading to a double shift.

The core problem is that your input `date` (YYYY-MM-DD) and `time` (HH:MM) strings are intended to represent a local time in `Asia/Makassar`, but `new Date()` might not interpret them that way, especially in a server environment that's likely set to UTC.

### **The Fix: Explicit Date Construction and Timezone Handling**

To ensure the `Date` objects are created unambiguously in the desired timezone, we should:

1.  **Parse date and time components numerically:** Extract year, month, day, hour, and minute as numbers.
2.  **Construct `Date` object without timezone offset:** Create the `Date` object using these numerical components. By default, this will create a `Date` object in the server's local timezone (often UTC on Render).
3.  **Crucially, Google Calendar API handles the timezone:** When you provide `timeZone: 'Asia/Makassar'` in the `start` and `end` objects of the Google Calendar event, Google's API will correctly interpret the `dateTime` (which is an ISO string in UTC) and display it correctly in the `Asia/Makassar` timezone for the event. The key is ensuring the `ISOString` represents the *correct point in UTC* that corresponds to your `Asia/Makassar` time.

I will update your `standalone-backend/server.js` Canvas to reflect this refined logic for constructing `startDate` and `endDate`.
